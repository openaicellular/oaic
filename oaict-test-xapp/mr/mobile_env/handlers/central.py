...
class MComCentralHandler(Handler):
    ...
    def reward(cls, env):
        ...
        utilities = np.asarray([utility for utility in env.utilities_scaled.values()])
        #print('utilities in handler.central=', utilities)
        ...
        #print('bounded in handler.central=', bounded)
        
        # return average utility of UEs to central agent as reward
        #print('np.mean(utilities) in handler.central=', np.mean(utilities))
